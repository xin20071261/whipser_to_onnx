...
#1、设置音频数据加载对应参数
LABEL_DIR = "./data/aishell/train/content.txt"#数据主目录，下面是语音子目录
...
SEED = 3407#随机数种子
DEVICE = "gpu" if torch.cuda.is_available() else "cpu"#设备
...
class WhisperModelModule(LightningModule):
    ...
    def val_dataloader(self):#加载验证数据
        ...
        return torch.utils.data.DataLoader(dataset, #pytorch数据加载函数
                          batch_size=self.cfg.batch_size, 

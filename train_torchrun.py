# -*- coding: utf-8 -*-
"""
Created on Mon Dec 11 15:52:18 2023
...
def ddp_setup():#ddp初始化
    """
    Args:
        rank: Unique identifier of each process
        world_size: Total number of processes
    """
    if int(os.environ["LOCAL_RANK"]) !=0:#保证第一个启动的是master节点
        time.sleep(1)
...
def prepare_val_dataloader(dataset: Dataset, batch_size: int):
    return DataLoader(
        dataset,
        batch_size=batch_size,
        pin_memory=True,
...
def main(save_every: int, total_epochs: int, batch_size: int, snapshot_path: str = "./model/snapshot.pt"):
    ddp_setup()
    dataset,test_dataset, model, optimizer,tokenizer  = load_train_objs()
    train_data = prepare_dataloader(dataset, batch_size)
    test_data = prepare_val_dataloader(test_dataset, batch_size)
    trainer = Trainer(model, train_data, test_data, optimizer,tokenizer, save_every,snapshot_path)

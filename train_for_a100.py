...
#1、设置音频数据加载对应参数
LABEL_DIR = "/ai/wqx/dataset/aishell/train/content.txt"#数据主目录，下面是语音子目录
...
SEED = 3407#随机数种子
DEVICE = "gpu" if torch.cuda.is_available() else "cpu"#设备,这个是linghtning的不同设置

#seed_everything(SEED, workers=True)#设置随机数
seed_everything(SEED)#设置随机数
...
def get_audio_file_list(label_dir, text_max_length=120, audio_max_sample_length=480000, sample_rate=16000):
    ...
    audio_paths = []
    path = "/ai/wqx/dataset/aishell/train/wav"#这个在不同的平台记得改
...
#8、训练模型
class WhisperModelModule(LightningModule):#一
    ...
    def training_step(self, batch, batch_id):# 训练
        ...
        labels = batch["labels"].long()#decoder输出预期值
    ...
    def configure_optimizers(self):
        """创建一个优化器和调度器"""
        """pytorch_lightning有默认的optimizer，用户定义时要先关闭，这个函数没有用"""
        model = self.model
    ...
    def train_dataloader(self):#加载训练数据,这个目前和pytorch写推理基本一致
        """创建训练数据加载器"""
...
#参考 https://zhuanlan.zhihu.com/p/440961384
trainer = Trainer(#一
    precision=16,
    accelerator=DEVICE,#选择使用什么设备
    max_epochs=cfg.num_train_epochs,
    accumulate_grad_batches=cfg.gradient_accumulation_steps,
    logger=tflogger,
    callbacks=callback_list,
    devices = 4,#-1表示使用所有gpu，4表示4个GPU
    strategy="ddp",#https://zhuanlan.zhihu.com/p/560121270
